...
CREATE POLICY "Allow auth insert members" ON members FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id OR user_id IS NULL);
CREATE POLICY "Allow auth update members" ON members FOR UPDATE TO authenticated USING (auth.uid() = user_id);
